...
export class DynamicFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.formService.getFormData(this.id).subscribe(
      (res: any) => {
        ...
        this.formDataById.fields_id.forEach((field, index) => {
          ...
          if (!field.is_visible_in_entry) {
            this.isVisibleInEntry.push(field.field_name);
          }

          this.formDataById.fields_id[index] = fieldBase;
        });

        this.form = this.formValidationService.toFormGroup(
          this.formDataById.fields_id
        );
  ...
  loadData() {
    ...
    this.formService.getFromAndEntriesById(this.id, this.entryId).subscribe(
      (res: any) => {
        this.isLoading = false;
        ...
        for (const key in this.profileData.value) {
          if (
            Object.prototype.hasOwnProperty.call(this.profileData.value, key)
          ) {
  ...
  onChange(event: any, type: any, fieldName?: any) {
    ...
    if (type === "email" || type === "password" || type === "text") {
      const indexOfObj: any = this.findIndex(
        this.formDataById.fields_id,
        event.target.name
      );
      if (indexOfObj >= 0) {
    ...
    if (type === "checkbox") {
      ...
      if (indexOfObj >= 0) {
        updatedFields[indexOfObj].value =
          typeof updatedFields[indexOfObj].value !== "undefined"
            ? [...updatedFields[indexOfObj].value, event.target.value]
            : [event.target.value];
        this.formDataById.fields_id = updatedFields;
      }
  ...
  findIndex(obj: Array<{ field_name: string }>, fieldName: string) {
    let returnIndex = -1;
  ...
  onSubmit() {
    ...
    if (authObj.user_role !== "ROLE_USER") {
      ...
      if (this.form.valid) {
        ...
        fd.append("is_visible_in_entry", JSON.stringify(this.isVisibleInEntry));
        fd.append("form_id", this.formDataById._id);
        fd.append("value", JSON.stringify(fieldData));

        this.formService.submitFormFieldsValue(fd).subscribe(
          (res: any) => {
            if (res.success == true) {
              this.isLoading = false;
              ...
              this.formService.showError(
                this.messageContent.errorMessage,
        ...
        this.formService.showError(
          this.messageContent.errorForRequiredFields,
          this.messageContent.errorMessageTitle
        );
      }
    } else {
      this.formService.showError(
        this.messageContent.permissionErrorRoleType,
        this.messageContent.errorMessageTitle
      );
    }
  }
  ...
  onUpdate() {
    ...
    if (this.form.valid) {
      ...
      fd.append("entry_id", this.entryId);
      fd.append("value", JSON.stringify(fieldData));

      this.formService.updateFormEntry(fd).subscribe(
        (res: any) => {
          if (res.success == true) {
            this.formService.showSuccess(
              this.messageContent.successMessageForUpdate,
