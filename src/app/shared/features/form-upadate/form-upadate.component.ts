...
export class FormUpdateComponent implements OnInit {
    ...
    this.formService.getFormData(this.id).subscribe(
      (res: any) => {
        ...
        this.UpdateFormDataById.fields_id.forEach((element, index) => {
          ...
          const removeProperty = (propKey, { [propKey]: propValue, ...rest }) =>
            rest;
          ...
          const removeProperties = (object, ...keys) =>
            keys.length
              ...
              : object;
          this.UpdateFormDataById.fields_id[index] = removeProperties(
            element,
            ...propKey
          );
        });
  ...
  createFormController() {
    this.UpdateFormDataById.fields_id.forEach((field, index) => {
      ...
      fieldBase.is_visible_in_entry = field.is_visible_in_entry || true;
    ...
    this.form = this.formValidationService.toFormGroup(
      this.UpdateFormDataById.fields_id
    );
  }

  updatedFiledValue(fieldValue: any) {
    let index = this.UpdateFormDataById.fields_id.findIndex(
      (x) => x._id === fieldValue._id
    );
  ...
  updateField(fieldValue, type) {
    ...
    this.form.controls["submitButtonName"].setValue(
      this.UpdateFormDataById.submitButtonName
    );
  ...
  saveField(fieldValue, type) {
    ...
    this.UpdateFormDataById.submitButtonName =
      this.form.controls["submitButtonName"].value;
  ...
  onSubmit() {
    ...
    this.formService.updateFormFieldsValue(newBody).subscribe(
      (res: any) => {
        this.isLoading = false;
