...
export class HeaderComponent implements OnInit {
  ...
  onFileChanged(event) {
    ...
    this.authService.UpdateProfileImage(uploadData).subscribe(
      (res) => {
        if (res) {
          this.formService.showSuccess(
            this.messageContent.UploadProfile,
  ...
  ngOnInit() {
    ...
    this.firstLatter = this.authObj.name
      ? this.authObj.name.charAt(0).toUpperCase()
      : "";
    this.getRandomColor();
  }
  ...
  logoutUser() {
    this.formService.logout().subscribe(
      (res) => {
        ...
        localStorage.clear();
        this.formService.showSuccess(
          this.messageContent.logoutSuccessMessage,
          this.messageContent.successMessageTitle
        );
      },
      (err) => {
